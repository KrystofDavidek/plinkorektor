...
export const setHovers = (token, pos, parId, isPopover, chunk, mistakeId, width) => {
  let tokens;
  ...
  for (const token of tokens) {
    ...
    if (width > 770) {
      ...
      $(token).on('click', () => {
        const cardElement = $(card).get()[0];
        if (cardElement) cardElement.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });
        $(card).addClass('selected');
      });
    }
